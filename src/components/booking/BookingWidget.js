...
export default function BookingWidget({ club }) {
    const [date, setDate] = useState("");
    const [selectedTable, setSelectedTable] = useState(null);
    const [bottles, setBottles] = useState([]);

    const handleBottleAdd = (bottle) => {
                ...
                <div className={styles.grid}>
                    {club.menu.map(item => (
                        <button
                            ...
                            style={item.isHotDeal ? { border: '1px solid var(--secondary)', background: 'rgba(255, 0, 85, 0.1)' } : {}}
                        >
                            <div style={{ fontSize: '0.85rem' }}>
                                {item.name}
                                {item.isHotDeal && <span style={{ fontSize: '0.7em', marginLeft: '4px', verticalAlign: 'top', color: 'var(--secondary)' }}>🔥</span>}
                            </div>
